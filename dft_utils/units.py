...
class Units:
    ...
    constants["Bohr_radius"] = 0.529177210544
    constants["Avogadro_constant"] = constants["NA"] = 6.02214076e23
    constants["speed_of_light"] = constants["c"] = 299792458
    constants["electron_charge"] = constants["e"] = 1.602176634e-19
    constants["Boltzmann"] = constants["k_B"] = 1.380649e-23
    constants["reduced_Planck_constant"] = constants["hbar"] = 1.054571817e-34
    constants["vacuum_electric_permittivity"] = constants["eps0"] = 8.8541878128e-12
    constants["vacuum_magnetic_permeability"] = constants["mu0"] = 1.0 / (
        constants["eps0"] * constants["c"] ** 2
    )
    ...
    distance["cm"] = distance["A"] * 1e-8
    distance["inch"] = distance["A"] / 2.54e-8
    ...
    @classmethod
    def nconvert(cls, value: T, inp: str, out: str, power: int) -> T:
        """Scale a value by a unit conversion factor raised to ``power``.

        This is used for quantities carrying an additional tensor length
        rank, such as quadrupoles and gradients. ``inp`` and ``out`` must be
        units of the same physical quantity.
        """
        if value is None or power == 0 or inp == out:
            return value
        ratio = cls.conversion_ratio(inp, out) ** power
        if isinstance(value, collections.abc.Iterable) and not isinstance(
            value, (str, bytes)
        ):
            if isinstance(value, numpy.ndarray):
                return value * ratio  # type: ignore[return-value]
            return type(value)(item * ratio for item in value)  # type: ignore[return-value]
        return value * ratio  # type: ignore[operator,return-value]
